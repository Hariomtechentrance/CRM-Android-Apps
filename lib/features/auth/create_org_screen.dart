...
class _CreateOrgScreenState extends ConsumerState<CreateOrgScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final orgData = res.data['data'] as Map<String, dynamic>;
      // Build Organization from API response, falling back to form values for any
      // fields the backend may not echo back (e.g. currency, role).
      final org = Organization(
        id:             orgData['id'] as String,
        name:           orgData['name'] as String? ?? _nameCtrl.text.trim(),
        slug:           orgData['slug'] as String? ?? '',
        role:           orgData['role'] as String? ?? 'OWNER',
        logo:           orgData['logo'] as String?,
        currency:       orgData['currency'] as String? ?? _currency,
        enabledModules: List<String>.from(orgData['enabledModules'] as List? ?? []),
      );
      // Persist org to user data and active org ID, then update in-memory state.
      // Both writes happen inside markOrgCreated so they're always in sync.
      await ref.read(authNotifierProvider.notifier).markOrgCreated(org);
      // Router auto-redirects to /home/dashboard via redirect logic
    } catch (e) {
      if (!mounted) return;
