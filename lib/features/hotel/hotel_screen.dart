...
class _HotelScreenState extends ConsumerState<HotelScreen> with SingleTickerProviderStateMixin {
      ...
      builder: (_) => _NewBookingSheet(
        ...
        onBooked: () {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Booking confirmed'), backgroundColor: AppColors.success, behavior: SnackBarBehavior.floating,
          ));
        },
      ),
    );
  ...
  Future<void> _checkIn(Map<String, dynamic> room) async {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _CheckInSheet(
        room: room,
        onCheckedIn: (guestName) {
          setState(() {
            final idx = _rooms.indexWhere((r) => r['no'] == room['no']);
            if (idx >= 0) _rooms[idx] = {..._rooms[idx], 'status': 'OCCUPIED', 'guest': guestName};
          });
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('$guestName checked in to Room ${room['no']}'),
            backgroundColor: _hotelColor, behavior: SnackBarBehavior.floating,
          ));
        },
      ),
    );
  ...
  Future<void> _checkOut(Map<String, dynamic> room) async {
    Navigator.pop(context);
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Check Out'),
        content: Text('Check out ${room['guest']} from Room ${room['no']}?\n\nTotal: ₹${room['rate']}'),
        actions: [
    ...
    try {
      final bookingId = _bookings.firstWhere(
        (b) => b['room'] == room['no'] && (b['status'] == 'CHECKED_IN'),
        orElse: () => {'id': room['no']},
      )['id'] as String;
      await ApiClient().checkOutGuest(bookingId, {'paymentMode': 'CASH', 'amount': room['rate']});
      if (!mounted) return;
      setState(() {
        final idx = _rooms.indexWhere((r) => r['no'] == room['no']);
        if (idx >= 0) _rooms[idx] = {..._rooms[idx], 'status': 'CLEANING', 'guest': null};
        _bookings.removeWhere((b) => b['room'] == room['no'] && b['status'] == 'CHECKED_IN');
      });
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Checked out from Room ${room['no']}'),
        backgroundColor: AppColors.success, behavior: SnackBarBehavior.floating,
      ));
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Check-out failed'), backgroundColor: AppColors.danger, behavior: SnackBarBehavior.floating,
      ));
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.add), onPressed: _showNewBookingSheet),
        ],
        bottom: TabBar(
          controller: _tabs,
          isScrollable: true,
...
class _CheckInSheet extends StatefulWidget {
  ...
  final void Function(String guestName) onCheckedIn;
  const _CheckInSheet({required this.room, required this.onCheckedIn});
  @override
  State<_CheckInSheet> createState() => _CheckInSheetState();
}

class _CheckInSheetState extends State<_CheckInSheet> {
  static const _hotelColor = Color(0xFF0EA5E9);
  ...
  @override
  void dispose() { _guestCtrl.dispose(); _phoneCtrl.dispose(); _checkOutCtrl.dispose(); super.dispose(); }

  Future<void> _submit() async {
    if (_guestCtrl.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Guest name required'), behavior: SnackBarBehavior.floating));
      return;
    }
    setState(() => _loading = true);
    try {
      final res = await ApiClient().createHotelBooking({
        'guestName':  _guestCtrl.text.trim(),
        'phone':      _phoneCtrl.text.trim(),
        'roomNo':     widget.room['no'],
        'checkInDate': DateTime.now().toIso8601String().split('T').first,
        'checkOutDate': _checkOutCtrl.text.trim().isNotEmpty ? _checkOutCtrl.text.trim() : null,
        'rate': widget.room['rate'],
      });
      final bookingId = res.data['data']?['id'] as String?;
      if (bookingId != null) {
        await ApiClient().checkInGuest(bookingId);
      }
      if (!mounted) return;
      Navigator.pop(context);
      widget.onCheckedIn(_guestCtrl.text.trim());
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Check-in failed'), backgroundColor: AppColors.danger, behavior: SnackBarBehavior.floating,
      ));
    } finally {
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Text('Check In — Room ${widget.room['no']} (${widget.room['type']})', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800)),
          const SizedBox(height: 4),
          Text('Rate: ₹${widget.room['rate']}/night', style: const TextStyle(fontSize: 12, color: AppColors.textGhost)),
          const SizedBox(height: 16),
          TextField(
            controller: _guestCtrl,
            textCapitalization: TextCapitalization.words,
            decoration: const InputDecoration(labelText: 'Guest Name *', prefixIcon: Icon(Icons.person_outlined, size: 18, color: AppColors.textGhost)),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: _phoneCtrl,
            keyboardType: TextInputType.phone,
            decoration: const InputDecoration(labelText: 'Phone', prefixIcon: Icon(Icons.phone_outlined, size: 18, color: AppColors.textGhost)),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: _checkOutCtrl,
            decoration: const InputDecoration(labelText: 'Check-out Date (YYYY-MM-DD)', prefixIcon: Icon(Icons.calendar_today_outlined, size: 18, color: AppColors.textGhost)),
          ),
          const SizedBox(height: 20),
          ElevatedButton.icon(
            onPressed: _loading ? null : _submit,
            style: ElevatedButton.styleFrom(backgroundColor: _hotelColor, foregroundColor: Colors.white, minimumSize: const Size.fromHeight(46)),
            ...
            label: const Text('Confirm Check In', style: TextStyle(fontWeight: FontWeight.w700)),
          ),
        ]),
      ),
    );
...
class _NewBookingSheet extends StatefulWidget {
  ...
  const _NewBookingSheet({required this.rooms, required this.onBooked});
  @override
...
class _NewBookingSheetState extends State<_NewBookingSheet> {
  ...
  @override
  void dispose() { _guestCtrl.dispose(); _checkInCtrl.dispose(); _checkOutCtrl.dispose(); super.dispose(); }

  Future<void> _submit() async {
    if (_guestCtrl.text.trim().isEmpty || _selectedRoom == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Guest name and room required'), behavior: SnackBarBehavior.floating));
      return;
    }
    setState(() => _loading = true);
    try {
      await ApiClient().createHotelBooking({
        ...
        'checkInDate': _checkInCtrl.text.trim(),
        'checkOutDate': _checkOutCtrl.text.trim(),
      });
      if (!mounted) return;
      Navigator.pop(context);
      widget.onBooked();
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Booking failed'), backgroundColor: AppColors.danger, behavior: SnackBarBehavior.floating,
      ));
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  ...
  @override
  Widget build(BuildContext context) {
    final availableRooms = widget.rooms.where((r) => r['status'] == 'AVAILABLE').toList();
    return Padding(
      padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom),
      child: SingleChildScrollView(
        padding: const EdgeInsets.fromLTRB(20, 20, 20, 30),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('New Booking', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          ...
          DropdownButtonFormField<String>(
            ...
            items: availableRooms.map((r) => DropdownMenuItem(
              value: r['no'] as String,
              child: Text('Room ${r['no']} (${r['type']}) — ₹${r['rate']}/night'),
            )).toList(),
          ...
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Check-in Date (YYYY-MM-DD)', prefixIcon: Icon(Icons.login_outlined, size: 18, color: AppColors.textGhost)),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: _checkOutCtrl,
            decoration: const InputDecoration(labelText: 'Check-out Date (YYYY-MM-DD)', prefixIcon: Icon(Icons.logout_outlined, size: 18, color: AppColors.textGhost)),
          ),
          const SizedBox(height: 20),
          ElevatedButton.icon(
            onPressed: _loading ? null : _submit,
            style: ElevatedButton.styleFrom(backgroundColor: _hotelColor, foregroundColor: Colors.white, minimumSize: const Size.fromHeight(46)),
            icon: _loading ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.book_online),
            label: const Text('Confirm Booking', style: TextStyle(fontWeight: FontWeight.w700)),
          ),
        ]),
      ),
    );
