...
import '../features/store/add_goods_entry_screen.dart';
import '../features/pos/pos_screen.dart';
import '../features/import_export/import_export_screen.dart';
import '../features/retail/retail_screen.dart';
import '../features/deals/deals_screen.dart';
import '../features/deals/add_deal_screen.dart';
...
import '../features/telecalling/telecalling_screen.dart';
import '../features/services/services_screen.dart';
import '../features/stock_market/stock_market_screen.dart';
import '../features/health/health_screen.dart';
import '../features/health/add_patient_screen.dart';
import '../features/lead_forms/lead_forms_screen.dart';
import '../features/lead_forms/create_lead_form_screen.dart';
...
class _RouterNotifier extends ChangeNotifier {
  ...
  String? redirect(BuildContext context, GoRouterState state) {
    ...
    if (authState is AsyncLoading) {
      // Don't redirect away from auth screens during login/register — the widget
      // handles its own loading spinner and needs to stay mounted to show errors.
    ...
    final isLoginFlow = ['/login', '/register', '/forgot-password'].contains(location);

    if (!isAuth) return isLoginFlow ? null : '/login';
    if (!hasOrg && location != '/create-org') return '/create-org';
    if (isLoginFlow || location == '/splash' || (location == '/create-org' && hasOrg)) return '/home/dashboard';
    return null;
  }
}
...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: '/store/add',          builder: (_, __) => const AddGoodsEntryScreen()),
      GoRoute(path: '/pos',                builder: (_, __) => const PosScreen()),
      GoRoute(path: '/import-export',      builder: (_, __) => const ImportExportScreen()),
      GoRoute(path: '/retail',             builder: (_, __) => const RetailScreen()),
      GoRoute(path: '/deals',              builder: (_, __) => const DealsScreen()),
      GoRoute(path: '/deals/add',          builder: (_, __) => const AddDealScreen()),
      GoRoute(path: '/deals/:id',          builder: (_, s)  => DealDetailScreen(id: s.pathParameters['id']!)),
      GoRoute(path: '/quotations',         builder: (_, __) => const QuotationsScreen()),
      GoRoute(path: '/email',              builder: (_, __) => const EmailScreen()),
      GoRoute(path: '/telecalling',        builder: (_, __) => const TelecallingScreen()),
      GoRoute(path: '/services',           builder: (_, __) => const ServicesScreen()),
      GoRoute(path: '/stock-market',       builder: (_, __) => const StockMarketScreen()),
      GoRoute(path: '/health',             builder: (_, __) => const HealthScreen()),
      GoRoute(path: '/health/add-patient', builder: (_, __) => const AddPatientScreen()),
      GoRoute(path: '/lead-forms',         builder: (_, __) => const LeadFormsScreen()),
      GoRoute(path: '/lead-forms/create',  builder: (_, __) => const CreateLeadFormScreen()),
